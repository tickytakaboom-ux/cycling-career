...
function RiderView({rider}:{rider:Rider}) {
  return <div className="content-view"><section className="panel profile-summary"><div className="rider-mark large">{rider.firstName[0]}{rider.lastName[0]}</div><div><span className="eyebrow">{profileLabels[rider.profile]} · {rider.nationality}</span><h1>{rider.firstName} {rider.lastName}</h1><p>{rider.height} cm · {rider.weight} kg · {rider.age} ans</p></div></section><section className="panel"><div className="panel-title"><h3>Caractéristiques</h3><span className="eyebrow">NIVEAU ACTUEL</span></div><div className="stats-grid">{Object.entries(rider.stats).map(([k,v])=><Metric key={k} label={statLabels[k as keyof RiderStats]} value={v}/>)}</div></section></div>;
}
...
function Dashboard({game,setGame,onReset}:{game:GameState;setGame:(g:GameState)=>void;onReset:()=>void}) {
  ...
  const trainingDone=game.lastTrainingDate===game.currentDate;
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      {tab==='dashboard' && <>
        <section className="hero-card"><div className="rider-mark">{game.rider.firstName[0]}{game.rider.lastName[0]}</div><div className="rider-title"><span>{game.team.name}</span><h1>{game.rider.firstName}<br/><strong>{game.rider.lastName}</strong></h1><div className="chips"><i>{game.rider.age} ANS</i><i>{game.rider.nationality.toUpperCase()}</i><i>{profileLabels[game.rider.profile].toUpperCase()}</i></div></div><div className="condition"><Metric label="FORME" value={game.rider.form} tone="#c9ff3d"/><Metric label="FATIGUE" value={game.rider.fatigue} tone="#ff7657"/><Metric label="MORAL" value={game.rider.morale} tone="#6fa8ff"/></div></section>
        <div className="dashboard-grid">
          ...
          <section className="panel training"><div className="panel-title"><div><span className="eyebrow">PROGRAMME DU JOUR</span><h3>Entraînement</h3></div><Activity/></div><div className="training-list">{(Object.keys(trainingConfig) as TrainingType[]).map(k=><button title={trainingConfig[k].description} className={game.selectedTraining===k?'active':''} onClick={()=>update({...game,selectedTraining:k})} key={k}><span>{trainingConfig[k].label}</span><small>{trainingConfig[k].fatigue>0?`+${trainingConfig[k].fatigue}`:trainingConfig[k].fatigue} fatigue · {trainingConfig[k].stats.length?`${trainingConfig[k].stats.length} axes travaillés`:'repos actif'}</small></button>)}</div><p className="training-note">Une seule séance par jour. La progression est lente ; enchaîner les charges dégrade la forme et augmente la fatigue.</p><div className="actions"><button className="primary" disabled={trainingDone||Boolean(available)} onClick={()=>update(train(game,game.selectedTraining))}>{available?'COURSE À DISPUTER':trainingDone?'SÉANCE EFFECTUÉE':'EFFECTUER LA SÉANCE'}</button><button className="outline" disabled={Boolean(available)} onClick={()=>update(nextDay(game))}>{available?'COURSE À DISPUTER':'JOUR SUIVANT'}</button></div></section>
        </div>

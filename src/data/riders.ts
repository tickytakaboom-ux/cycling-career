...
const bonuses:Record<RiderProfile,Partial<RiderStats>>={grimpeur:{mountain:14,climbing:12,endurance:5,recovery:4,sprint:-6,pavement:-3},puncheur:{climbing:9,explosiveness:14,sprint:5,endurance:4},sprinteur:{sprint:16,explosiveness:11,endurance:4,mountain:-8,climbing:-5},rouleur:{timeTrial:15,endurance:10,resistance:7,tactics:5},baroudeur:{endurance:10,resistance:10,explosiveness:7,tactics:8},classiqueur:{pavement:15,resistance:9,weather:7},etapes:{mountain:7,climbing:6,timeTrial:7,endurance:8,recovery:5}};
const potentialBonuses:Record<RiderProfile,Partial<RiderStats>>={grimpeur:{mountain:10,climbing:9,endurance:4,recovery:4},puncheur:{climbing:6,explosiveness:10,sprint:4,endurance:3},sprinteur:{sprint:11,explosiveness:8,endurance:3},rouleur:{timeTrial:11,endurance:7,resistance:5,tactics:4},baroudeur:{endurance:8,resistance:8,explosiveness:5,tactics:6},classiqueur:{pavement:11,resistance:7,weather:5},etapes:{mountain:6,climbing:5,timeTrial:6,endurance:7,recovery:5}};
...
const profileKeys:Record<RiderProfile,(keyof RiderStats)[]>={grimpeur:['mountain','climbing','endurance','recovery'],puncheur:['climbing','explosiveness','sprint','endurance'],sprinteur:['sprint','explosiveness','endurance'],rouleur:['timeTrial','endurance','resistance','tactics'],baroudeur:['endurance','resistance','explosiveness','tactics'],classiqueur:['pavement','resistance','weather'],etapes:['mountain','climbing','timeTrial','endurance','recovery']};
export const opponents=names.map((name,index)=>{
  const age=20+index%13;
  const rider=createRider({firstName:name[0],lastName:name[1],nationality:'Europe',height:174+index%12,weight:62+index%13,profile:profiles[index%profiles.length]});
...
export function generateOpponents(targetLevel:number,count=30):Rider[]{
 return Array.from({length:count},(_,index)=>{
  const template=opponents[index%opponents.length],elite=index>=count-3;
  const level=targetLevel+(elite?3+(index-(count-3))*1.5:((index*7)%13-6)*.65);
  const delta=level-Object.values(template.stats).reduce((sum,value)=>sum+value,0)/Object.keys(template.stats).length;
  const stats={...template.stats};Object.keys(stats).forEach(key=>stats[key as keyof RiderStats]=clamp(stats[key as keyof RiderStats]+delta,32,88));
  return {...template,id:`field-${index}-${targetLevel}`,stats,age:20+index%14,experience:160+(index%12)*120,form:56+index%17,fatigue:5+index%18};
 });
}
